...
function library_search($search) {
/*
  $search_text=ltrim($search);
  $search_text=rtrim($search);
...
*/
  $db = db::$connection;
  $sql = "SELECT title, author, year, location, call_number FROM book WHERE title LIKE '% $search %'";
  $stmt = $db->prepare($sql);
  //$stmt->bind_param("s", $q);
  $stmt->bind_result($title, $author, $year, $location, $call_number);
  $stmt->execute();  
  $books = array();
  while($stmt->fetch()) {
	$books[] = array(
...
function lookup_booktitle($title) {
	$db = db::$connection;
	$sql = "SELECT title, author, year, location, call_number FROM book WHERE title=?";
	$stmt = $db->prepare($sql);
...
*/
	$sr = ldap_search($ds, "o=University of Notre Dame,st=Indiana,c=US", $query->out())
         or ldap_die("could not search");
